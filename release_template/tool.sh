...
cp_doc(){
    echo_eval cp doc/{{name}}_api.html apps/{{name}}/priv/static/
}
set_var(){
    # cp_static
    # cp_doc
    export {{NAME}}_LOG_ROOT="logs"
	export {{NAME}}_LOG_LEVEL="error"
    export {{NAME}}_PORT="5001"
    export {{NAME}}_NODE_NAME="{{name}}@127.0.0.1"
    export {{NAME}}_COOKIE="xxx"
    export {{NAME}}_REDIS_IP="127.0.0.1"
    export {{NAME}}_REDIS_PORT="6379"
    export {{NAME}}_REDIS_PASSWORD="123456"
    export {{NAME}}_MYSQL_IP="localhost"
    export {{NAME}}_MYSQL_PORT="3306"
    export {{NAME}}_MYSQL_DB="{{name}}"
    export {{NAME}}_MYSQL_PASSWORD="123456"
    export {{NAME}}_MYSQL_USER="root"
}
gen_commit_id(){
...
release) release $2;;
tar) release $2
     croc send --code xxxx _build/prod/rel/{{name}}/{{name}}-*.tar.gz;;
